...
public class RegionRouteTable {
    ...
    public void addOrUpdateRegion(final Region region, byte type) {
        Requires.requireNonNull(region, "region");
        Requires.requireNonNull(region.getRegionEpoch(), "regionEpoch");
        final long regionId = region.getId();
        System.out.println("add region : " + regionId);
        final byte[][] startEndKey = getStartEndKeyByType(region, type);
        final StampedLock stampedLock = this.stampedLock;
        final long stamp = stampedLock.writeLock();
        try {
            this.regionTable.put(regionId, region.copy());
            this.rangeTable.put(startEndKey[0], regionId);
        } finally {
            stampedLock.unlockWrite(stamp);
        }
    }
    ...
    protected Region findRegionByKeyWithoutLock(final byte[] key) {
    ...
    private byte[][] getStartEndKeyByType(Region region, byte type) {
